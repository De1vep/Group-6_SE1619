...
@WebServlet(name="QuizHandleControllerr", urlPatterns={"/view/quizHandleController"})
public class QuizHandleController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            QuizQuizHandleDAO quizQHInterface = new QuizQuizHandleDAO();
            QuestionQuizHandleDAO questionQHInterface = new QuestionQuizHandleDAO();
            ...
            if (service.equalsIgnoreCase("quizEntrance")) {
                ...
                if (session.getAttribute("doingQuiz") == null) {
                    ...
                    QuizQuizHandle doingQuiz = quizQHInterface.generateQuiz(questionList, 1, account);
                    response.getWriter().print(doingQuiz.getTime());
//                    session.setAttribute("doingQuiz", doingQuiz);
                }
                //response.sendRedirect("quizHandleController?service=quizHandle&quizId=" + 1 + "&questionNumber=1");
            }
